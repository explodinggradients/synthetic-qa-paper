...
class FAISS(VectorStore):
    ...
    def batch_similarity_search(
        ...
        """
        # faiss = dependable_faiss_import()
        if verbose: print(f'- Fetching Embeddings ({len(queries)} Queries)')
        embeddings = self._embed_documents(queries)
        vectors = np.array(embeddings, dtype=np.float32)
        if verbose: print('- Searching Vector Index')
        _, I = self.index.search(vectors, k)
        if verbose: print('- Fetching Similar Documents')
        ls_docs = []
        for i in range(I.shape[0]):
            docs = []
            for j in range(I.shape[1]):
                ind = I[i, j]
                doc_ind = self.index_to_docstore_id[ind]
                doc = self.docstore.search(doc_ind)
                docs.append(doc)
            ls_docs.append(docs)
